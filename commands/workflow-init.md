...
2. **Create folders** (if missing): `<vault>/.claude/agentic-workflow/templates`,
   `<vault>/Tasks/Agentic/tickets`, `<vault>/Tasks/Agentic/logs`, `<vault>/Kanban`, and — when
   `learning` is on — `<vault>/<knowledgeFolder>/repos`.
   (Use `taskNotesFolder` / `logsFolder` / `knowledgeFolder` from config if customized.)
